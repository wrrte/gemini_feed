...
def test_init_loads_data(mock_storage_manager, mock_sensor_manager):
    ...
    mock_mode_schema.model_dump.return_value = {"mode_id": 1, "mode_name": "TestMode", "sensor_ids": []}
    mock_storage_manager.get_all_safehome_modes.return_value = [mock_mode_schema]
    ...
    mock_settings = Mock()
    mock_settings.model_dump.return_value = {
        "system_setting_id": 1, "panic_phone_number": "1", "homeowner_phone_number": "2",
...
def test_load_system_settings_not_found(mock_storage_manager, mock_sensor_manager):
...
def test_delete_safety_zone_not_in_cache(config_manager, mock_storage_manager):
    ...
    assert config_manager.delete_safety_zone(1) is True
    # Ensures 'if zone_id in self.safety_zones' evaluates to False without error
...
def test_zone_has_other_armed_sensors(config_manager, mock_sensor_manager):
    ...
    assert config_manager._zone_has_other_armed_sensors(zone, exclude_sensor_id=2) is True
    assert config_manager._zone_has_other_armed_sensors(zone, exclude_sensor_id=1) is False
...
def test_missing_sensor_manager_ops(mock_storage_manager):
    ...
    cm = ConfigurationManager(storage_manager=mock_storage_manager, sensor_manager=None)
