...
from database.schema.camera import (CameraControlType, CameraSchema,
...
def camera_manager(mock_storage_manager, mock_safe_home_camera_class):
    ...
    mock_camera_schema.model_dump.return_value = {
        "camera_id": 1, "enabled": True}
...
def test_delete_camera(camera_manager, mock_storage_manager,
                       camera_id, delete_success, expected_result):
...
def test_enable_cameras(camera_manager, mock_storage_manager,
                        ids, db_success, expected_result):
...
def test_toggle_all_cameras(
        camera_manager, mock_storage_manager, action, expected_method):
...
@pytest.mark.parametrize("control_type, move_success, "
                         + "db_success, expected_result", [
                             ...
                             (CameraControlType.PAN_RIGHT,
                              False, True, False),  # Move fails
                             (CameraControlType.PAN_RIGHT,
                              True, False, False),  # DB fails
                             # Invalid control enum
                             ("INVALID_CONTROL", True, True, False)
                         ])
def test_control_single_camera(camera_manager, mock_storage_manager,
                               control_type, move_success,
                               db_success, expected_result):
    cam = camera_manager.get_camera(1)
...
def test_control_single_camera_invalid_id(camera_manager):
    assert camera_manager.control_single_camera(
        999, CameraControlType.PAN_RIGHT) is False
...
def test_set_camera_password(
        camera_manager, mock_storage_manager, camera_id, should_update):
    cam = camera_manager.get_camera(1)
...
@pytest.mark.parametrize("camera_id, "
                         + "has_pwd, correct_pwd, input_pwd, expected", [
                             (1, True, "1234", "1234",
                             ...
                             (999, False, None, "any",
                              CameraValidationResult.INVALID_ID)
                         ])
def test_validate_camera_password(
        camera_manager, camera_id, has_pwd, correct_pwd, input_pwd, expected):
    cam = camera_manager.get_camera(1)
    cam.has_password.return_value = has_pwd
    cam.password = correct_pwd

    assert camera_manager.validate_camera_password(
        camera_id, input_pwd) == expected
...
def test_delete_camera_password(
        camera_manager, mock_storage_manager, camera_id, expected):
